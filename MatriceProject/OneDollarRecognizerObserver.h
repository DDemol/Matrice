...
public:
    Action action;
    double score;
    RecognitionResult(Action action, double score)
    {
        this->action = action;
        this->score = score;
    }
...
public:
    ...
    GestureTemplate(Action action, Path2D points)
    {
        this->action   = action;
        this->points = points;
    }
...
class OneDollarRecognizerObserver : public QObject, public Observer
{
    Q_OBJECT
private:
...
public:
    ...
    void loadTemplates();
    int addTemplate(Action action, Path2D points);
